...
public class InvInboundService extends ServiceImpl<InvInboundMapper, InvInbound> {
    ...
    public PageResult<InvInbound> pageInbounds(Long enterpriseId, Integer page, Integer size, String inboundNo, String sourceNo,
        ...
        wrapper.eq(InvInbound::getEnterpriseId, enterpriseId)
                ...
                .like(StrUtil.isNotBlank(sourceNo), InvInbound::getSourceNo, sourceNo)
    ...
    public void confirmInbound(Long id, Long enterpriseId, Long operatorId) {
        InvInbound inbound = baseMapper.selectForUpdate(id, enterpriseId);
        if (inbound == null) {
            throw new BusinessException("入库单不存在");
        }
