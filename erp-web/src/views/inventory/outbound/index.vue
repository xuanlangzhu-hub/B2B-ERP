<template>
  <div class="page-wrapper">
    <el-card>
      <div class="toolbar">
        <el-input v-model="query.outboundNo" placeholder="出库单号" clearable style="width:180px" />
        <el-input v-model="query.sourceNo" placeholder="销售/退货单号" clearable style="width:180px" />
        ...
        <el-button type="success" @click="handleOpenCreate">新增出库</el-button>
      </div>
    </el-card>
    <el-card style="margin-top:12px">
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="outboundType" label="出库类型" width="120">
          <template #default="{ row }">{{ outboundTypeLabel(row.outboundType) }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            ...
            <el-button size="small" type="success" v-if="row.status === 'DRAFT'" @click="handleConfirm(row)">确认出库</el-button>
    ...
    <el-dialog title="新增出库" v-model="createDialogVisible" width="700px">
      <el-table :data="salesOrderOptions" v-loading="optionsLoading" border stripe @selection-change="handleSelectSales"
        max-height="400">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleCreateOutbound" :disabled="!selectedSalesRow" :loading="creating">确定出库</el-button>
      </template>
    </el-dialog>

    <el-dialog title="出库详情" v-model="detailDialogVisible" width="700px">
...
import { getOutbounds, getOutbound, confirmOutbound, createOutboundFromSales, cancelOutbound } from '@/api/inventory'
import { getOutboundOptions } from '@/api/sales'
...
const query = reactive({
  page: 1, size: 10,
  outboundNo: '', sourceNo: '', warehouseId: '', status: '', dateRange: [] as string[]
})

function outboundTypeLabel(type: string) {
  return ({ SALES: '销售出库', PURCHASE_RETURN: '采购退货出库' } as Record<string, string>)[type] || type
}
...
async function fetchData() {
  ...
  try {
    ...
    const res = await getOutbounds(params)
...
async function handleConfirm(row: any) {
  await ElMessageBox.confirm('确定确认该出库单吗？系统将检查库存是否充足。', '确认出库', { type: 'warning' })
  try {
    ...
    if (err?.response?.data?.message) {
      ElMessage.warning('库存不足：' + err.response.data.message)
    }
  }
}

async function handleCancel(row: any) {
  await ElMessageBox.confirm('确定取消该出库单吗？', '确认取消', { type: 'warning' })
...
const selectedSalesRow = ref<any>(null)
...
async function handleOpenCreate() {
  ...
  try {
    const res = await getOutboundOptions()
    salesOrderOptions.value = res.data || []
  } finally { optionsLoading.value = false }
}

function handleSelectSales(rows: any[]) {
  selectedSalesRow.value = rows.length > 0 ? rows[0] : null
}

async function handleCreateOutbound() {
  if (!selectedSalesRow.value) return
  creating.value = true
  try {
    await createOutboundFromSales(selectedSalesRow.value.id)
