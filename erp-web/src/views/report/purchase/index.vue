  ...
  <div class="page-wrapper">
    ...
    <el-card style="margin-top:12px">
      ...
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="totalQuantity" label="总数量" width="100"/><el-table-column prop="totalAmount" label="总金额" width="110"/><el-table-column prop="paidAmount" label="已付金额" width="110"/>
      </el-table>
...
onMounted(fetchData)
async function fetchData(){loading.value=true;try{const params:any={...query};if(dateRange.value?.length===2){params.startDate=dateRange.value[0];params.endDate=dateRange.value[1]}const r=await getPurchaseOrders(params);tableData.value=r.data.records;total.value=r.data.total;summary.orderCount=r.data.total;summary.totalAmount=r.data.records?.reduce((s:number,i:any)=>s+Number(i.totalAmount||0),0)||0;summary.paidAmount=r.data.records?.reduce((s:number,i:any)=>s+Number(i.paidAmount||0),0)||0}finally{loading.value=false}}
</script>
