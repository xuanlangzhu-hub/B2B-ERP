  ...
  <div class="page-wrapper">
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="800px" @close="resetForm">
      <el-form :model="form" ref="formRef" label-width="100px">
        <el-row :gutter="16">
          <el-col :span="8"><el-form-item label="客户"><el-select v-model="form.customerId" placeholder="选择客户" filterable style="width:100%"><el-option v-for="c in customers" :key="c.value" :label="c.label" :value="c.value" /></el-select></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="出库仓库"><el-select v-model="form.warehouseId" placeholder="选择仓库" style="width:100%"><el-option v-for="w in warehouses" :key="w.value" :label="w.label" :value="w.value" /></el-select></el-form-item></el-col>
    ...
    <el-dialog title="销售单详情" v-model="detailVisible" width="800px">
...
import { getSalesOrders, getSalesOrder, createSalesOrder, updateSalesOrder, deleteSalesOrder, approveSalesOrder, cancelSalesOrder } from '@/api/sales'
...
const dialogVisible = ref(false); const dialogTitle = ref('新增销售单'); const editingId = ref<number|null>(null)
const formRef = ref(); const form = reactive({ customerId: null, warehouseId: null, orderDate: '', remark: '', items: [] as any[] } as any)
...
async function loadOptions() {
  const [c, w, p] = await Promise.all([getCustomerOptions(), getWarehouseOptions(), getProductOptions()])
  customers.value = c.data || []; warehouses.value = w.data || []
...
function handleAdd() { editingId.value = null; dialogTitle.value = '新增销售单'; resetForm(); dialogVisible.value = true }
function addItem() { form.items.push({ productId: null, quantity: 0, unitPrice: 0, amount: 0, unitId: null }) }
function onProductChange(row: any) { const p = products.value.find(x => x.value === row.productId); if (p) { row.productCode = p.productCode; row.productName = p.productName; row.unitPrice = p.salePrice || 0; row.unitId = p.unitId; calcRowAmount(row) } }
function calcRowAmount(row: any) { row.amount = +(row.quantity * row.unitPrice).toFixed(2) }

async function handleSubmit() {
  if (!form.customerId || !form.warehouseId || !form.orderDate) { ElMessage.warning('请填写客户、仓库和业务日期'); return }
  ...
  try {
    ...
    if (editingId.value) { await updateSalesOrder(editingId.value, payload) }
...
async function handleEdit(row: any) {
  ...
  const d = res.data; editingId.value = row.id; dialogTitle.value = '编辑销售单'
  Object.assign(form, { customerId: d.customerId, warehouseId: d.warehouseId, orderDate: d.orderDate, remark: d.remark, items: d.items || [] })
  dialogVisible.value = true
}

async function handleDetail(row: any) { const r = await getSalesOrder(row.id); detail.value = r.data; detailVisible.value = true }

async function handleApprove(row: any) { await ElMessageBox.confirm('确认审核该销售单？'); await approveSalesOrder(row.id); ElMessage.success('已审核'); fetchData() }
async function handleCancel(row: any) { await ElMessageBox.confirm('确认取消该销售单？'); await cancelSalesOrder(row.id); ElMessage.success('已取消'); fetchData() }
async function handleDelete(row: any) { await ElMessageBox.confirm('确认删除？'); await deleteSalesOrder(row.id); ElMessage.success('已删除'); fetchData() }

function resetForm() { Object.assign(form, { customerId: null, warehouseId: null, orderDate: new Date().toISOString().slice(0,10), remark: '', items: [] }) }
</script>
