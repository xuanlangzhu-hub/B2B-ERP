  ...
  <div class="page-wrapper">
    ...
    <el-card style="margin-top: 12px">
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="salesOrderNo" label="原销售单" width="165" />
        ...
        <el-table-column label="操作" width="300" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 'APPROVED'" size="small" type="primary" @click="generateInbound(row)">生成退货入库</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <el-dialog v-model="createVisible" title="新增销售退货申请" width="920px" @closed="resetForm">
      <el-alert
        title="当前版本支持冲减未收款的退货；若原销售单已全部收款，需要人工退款，系统会阻止提交。"
        type="info"
        :closable="false"
        show-icon
      ...
      <el-form :model="form" label-width="100px">
        <el-row :gutter="16">
          <el-col :span="9">
            <el-form-item label="原销售单" required>
              <el-select v-model="form.salesOrderId" filterable style="width: 100%" placeholder="选择已完成销售单" @change="loadOrderDetail">
                <el-option v-for="order in orderOptions" :key="order.id" :label="`${order.orderNo} / ${order.customerName || ''}`" :value="order.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="7">
    ...
    <el-dialog v-model="detailVisible" title="销售退货详情" width="820px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="原销售单">{{ detail.salesOrderNo }}</el-descriptions-item>
...
import { createInboundFromSalesReturn } from '@/api/inventory'
...
const form = reactive<any>({ salesOrderId: null, returnDate: today(), warehouseId: null, returnReason: '', remark: '', items: [] })
...
async function fetchData() {
  loading.value = true
  try {
    const res = await getSalesReturns(query)
...
async function openCreate() {
  ...
  try {
    const [orders, warehouseResult] = await Promise.all([
      getSalesOrders({ page: 1, size: 100, status: 'COMPLETED' }),
      getWarehouseOptions()
    ])
...
async function loadOrderDetail(orderId: number) {
  ...
  try {
    ...
    form.items = (order.items || []).map((item: any) => ({
      ...
      maxReturnQuantity: Math.max(0, Number(item.outboundQuantity || 0) - Number(item.returnedQuantity || 0)),
      returnQuantity: 0
    })).filter((item: any) => item.maxReturnQuantity > 0)
    if (form.items.length === 0) ElMessage.warning('该销售单没有可退商品')
  } finally { orderLoading.value = false }
}

async function submit() {
  const selected = form.items.filter((item: any) => Number(item.returnQuantity) > 0)
  if (!form.salesOrderId || !form.returnDate || !form.warehouseId) {
    ElMessage.warning('请选择原销售单、退货日期和退货仓库')
    return
  }
  ...
  try {
    await createSalesReturn({
      ...
      remark: form.remark,
      items: selected.map((item: any) => ({ salesOrderItemId: item.salesOrderItemId, quantity: item.returnQuantity }))
    })
    ElMessage.success('销售退货申请已保存')
    createVisible.value = false
    fetchData()
...
async function approve(row: any) {
  await ElMessageBox.confirm('审核后即可生成退货入库单，确定审核吗？', '审核退货申请', { type: 'warning' })
...
async function generateInbound(row: any) {
  await createInboundFromSalesReturn(row.id)
  ElMessage.success('退货入库单已生成，请到“入库管理”确认入库')
  fetchData()
}

function resetForm() {
  Object.assign(form, { salesOrderId: null, returnDate: today(), warehouseId: null, returnReason: '', remark: '', items: [] })
}
</script>
